...
- Configuration: `AccountProfile::preferred_rec_engine` (string).
...
1. Add `preferred_rec_engine` to `AccountProfile` in `domain`.
2. Wire account profile CRUD into `/api/v1/auth/me`.
3. In discovery handlers, merge `preferred_rec_engine` with config.
4. Add integration test: set user preference, trigger discovery, assert the engine’s ranking appears in the first result’s attribution.
