...
### Status (fifth review, 2026-09-19 — FK fixed, gate still red)

`f909775` fixed the SQLite foreign-key break (verified with the fourth pass's own
reproducer: the FK target is `export_jobs` again and a real `download_grants`
insert succeeds) and added the missing `/exports/bulk` inventory row. At that
commit in a clean worktree, `milestone_7` is 10/10 (was 9/10 while the FK was
broken) and `route_inventory` 2/2, with `cargo fmt` still red. What is
still open, in order:

1. **The bulk export still ignores its query.** `QueryAst` is the DSL enum built by
   `parse_query(&str)`; the route body is `Json<MediaQuery>` (a JSON object), so
   `as_str().unwrap_or_default()` yields `""` and every filter is dropped. Decide:
   DSL string or a `MediaQuery` → `QueryAst` bridge — and make an unreadable stored
   query `Fatal` rather than "export everything".
2. **PostgreSQL migration order**: `DROP TABLE export_jobs_old` precedes the drops
   of its dependents, so PG refuses the drop and the migration aborts. Unverified
   (no instance), flagged.
3. **The rebuild discards `download_grants` rows** (`DROP TABLE IF EXISTS` without a
   copy) — the sibling `export_jobs` rebuild copies; this one should.
4. **Gate red on two fronts**: `cargo fmt` wants two blocks in `bulk_export.rs`
   expanded and clippy fails `lorehaven-db` with three lints (untouched by the fix);
   and the *working tree does not compile* — `server.rs:159` omits `attempts` in an
   `OutboxEvent` literal (`error[E0063]`), so no test runs at all.
5. The fourth pass's shape gaps are untouched: ZIP served as `text/html`/`*.html`,
   no manifest, no checkpoint (duplicate item rows on retry), `load_subject` errors
   recorded as "skipped", no `has_entitlement`, no `ContentRead` scope, and no test
   for the export path.

**Phase 3a is in flight** (`crates/app/src/webhook_delivery.rs`, uncommitted):
delivery to subscribed endpoints with attempts recorded and wiring to the
`publish.notify` topic. Query watches (3b) — the half M23-02's webhook requirement
actually names — are still to come.
