...
### Status (reviewed 2026-09-19 — Phases 1 and 2 landed, gate red)

Six commits: `a6f0e20` (kinds via macro), `1a56da0` (migration 0035), `c6fc318`
(worker), `b949f9e` (route), `7badbcc` (fairness), `6be66b3` (webhook sender).

**Done and verified.** The kind list is generated by `job_kinds!` together with the
enum, so it cannot drift. Queue fairness has the right shape (`claim_sql`, class
filter, per-requester skip, bulk concurrency guard; 25 tests in `milestone_5`).
Signing is real HMAC-SHA256 with an RFC 4231 vector.

**Red gate and three blockers before anything else** (`docs/verification.md` has
the evidence and reproducers):

1. **Migration 0035 breaks every download grant.** The SQLite rebuild's rename
   leaves `download_grants.export_job_id` pointing at the dropped
   `export_jobs_old`; `milestone_7::the_download_grant_expires_and_is_single_use`
   fails with a 422 on minting. The PostgreSQL twin is unverified (no instance)
   and has the same shape. Gate: 1237 passed / 2 failed, `fmt` and `clippy` red.
2. **The bulk export ignores its query** — `if query_json.is_null() { None } else
   { None }` in `request_bulk` (`crates/app/src/exports.rs:792`) and `produce_bulk`
   (`crates/app/src/bulk_export.rs:180-184`). Today every bulk export means "all
   visible media", and over the cap it always fails.
3. **`/exports/bulk` has no row in the route inventory table**, which the direction
   test reports by name (and `fmt` wants two blocks expanded).

**Shape wrong for the artifact.** The row is `format: "html"`, and the download
route derives media type and extension from the format, so the ZIP arrives as
`*.html` / `text/html`.

**Gaps against this plan's own decisions.** No manifest in the bundle (D4); no
checkpoint and duplicate item rows on retry; every `load_subject` error recorded
as "skipped" (an infrastructure fault produces a quietly smaller, "ready" export);
`has_entitlement` still unconsulted (D3's second half); `ContentRead` not enforced
on the route; no test for the export path itself (the tests cited for M23-02 are
the fairness tests); the route is `/api/v1/exports/bulk`, not the `/api/v1/media/export`
this plan named.

**Webhooks: still not a pipeline.** The sender module exists and signs properly,
but nothing calls it — no delivery path, `record_delivery` without a production
caller — and its SSRF guard needs redirects disabled, every resolved address
checked, and IPv4-mapped IPv6 plus multicast/CGNAT ranges blocked before it can be
