...
pub fn router() -> axum::Router<AppState> {
    axum::Router::new()
        ...
        .route(
            "/admin/local-mirrors/{mirror_id}",
            delete(deactivate_local_mirror),
        )
        .route(
            "/media/references/{reference_id}/mirrors",
            get(list_local_mirrors),
        )
        .route(
            "/admin/dmca-takedowns",
            post(file_dmca_takedown).get(list_dmca_takedowns),
        )
        .route(
            "/admin/dmca-takedowns/{takedown_id}",
            put(resolve_dmca_takedown),
        )
        .route(
            "/media/references/{reference_id}/ipfs-pins",
            get(list_ipfs_pins).post(add_ipfs_pin),
        )
}
...
pub async fn add_local_mirror(
    State(state): State<AppState>,
    RequireSession(user): RequireSession,
    Json(payload): Json<AddLocalMirrorPayload>,
    ...
    lorehaven_db::media_resilience::insert_local_mirror(
        state.db(),
        &id,
        &payload.media_reference_id,
        &payload.storage_path,
        &payload.original_url,
        payload.file_size_bytes as i64,
...
) -> ApiResult<Json<Vec<LocalMirrorView>>> {
    let mirrors = lorehaven_db::media_resilience::list_local_mirrors(state.db(), &reference_id)
        .await
        .map_err(|e| ApiError(AppError::Internal(e.into())))?;
    let views: Vec<LocalMirrorView> = mirrors
        .iter()
        .map(|m| LocalMirrorView {
            id: m.id.clone(),
            media_reference_id: m.media_reference_id.clone(),
            storage_path: m.storage_path.clone(),
            original_url: m.original_url.clone(),
            file_size_bytes: m.file_size_bytes as u64,
            content_type: m.content_type.clone(),
            checksum_sha256: m.checksum_sha256.clone(),
            mirrored_by: m.mirrored_by.clone(),
            status: m.status.clone(),
            mirrored_at: m.mirrored_at.clone(),
        })
        .collect();
    Ok(Json(views))
}

pub async fn deactivate_local_mirror(
    State(state): State<AppState>,
    RequireSession(user): RequireSession,
...
pub async fn add_ipfs_pin(
    State(state): State<AppState>,
    RequireSession(user): RequireSession,
    Json(payload): Json<AddIpfsPinPayload>,
) -> ApiResult<impl axum::response::IntoResponse> {
    require_operator(&state, &user).await?;
    let id = Uuid::new_v4().to_string();
...
pub async fn list_ipfs_pins(
    ...
    Path(reference_id): Path<String>,
    _session: MaybeSession,
) -> ApiResult<Json<Vec<IpfsPinView>>> {
    let pins = lorehaven_db::media_resilience::list_ipfs_pins(state.db(), &reference_id)
        .await
        .map_err(|e| ApiError(AppError::Internal(e.into())))?;
    let views: Vec<IpfsPinView> = pins
        .iter()
        .map(|p| IpfsPinView {
            id: p.id.clone(),
            media_reference_id: p.media_reference_id.clone(),
            cid: p.cid.clone(),
            pin_service: p.pin_service.clone(),
            status: p.status.clone(),
            file_size_bytes: p.file_size_bytes as u64,
...
pub async fn list_dmca_takedowns(
    State(state): State<AppState>,
    RequireSession(user): RequireSession,
...
pub async fn resolve_dmca_takedown(
    State(state): State<AppState>,
    RequireSession(user): RequireSession,
    Path(takedown_id): Path<String>,
    Json(payload): Json<ResolveDmcaPayload>,
) -> ApiResult<StatusCode> {
    require_operator(&state, &user).await?;
