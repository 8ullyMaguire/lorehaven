...
async fn the_media_feed_supports_jsonld_format() {
    ...
    assert!(
        body.contains("\"CreativeWork\""),
        "missing CreativeWork type: {body}"
    );
    assert!(
        body.contains("JSON-LD Test Work"),
        "missing work title: {body}"
    );
    assert!(body.contains("\"url\""), "missing url field: {body}");
    assert!(
        body.contains("\"dateModified\""),
        "missing dateModified: {body}"
    );
...
async fn media_quality_and_date_filters() {
    ...
    assert_eq!(
        items.len(),
        2,
        "A and B have editorial_review signal >= 30: {body}"
    );
    ...
    assert!(
        items.iter().any(|i| i["id"] == work_c),
        "work C should appear: {body}"
    );
    assert!(
        !items.iter().any(|i| i["id"] == work_a),
        "work A should not appear: {body}"
    );
    ...
    assert_eq!(
        status,
        StatusCode::UNPROCESSABLE_ENTITY,
        "should reject quality_min without kind: {body}"
    );
    ...
    assert_eq!(
        status,
        StatusCode::UNPROCESSABLE_ENTITY,
        "should reject invalid date: {body}"
    );
    assert_eq!(body["error"]["code"], "VALIDATION_FAILED");
