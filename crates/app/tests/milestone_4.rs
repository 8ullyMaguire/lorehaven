...
async fn register(client: &mut Client, email: &str, handle: &str) -> String {
    ...
    body["account"]["id"].as_str().expect("account id").to_owned()
}
...
async fn save_chapter(client: &mut Client, chapter: &str, version: i64, text: &str) -> (StatusCode, Value) {
    client
        .patch(
...
async fn a_visitor_can_read_a_published_chapter() {
    ...
    save_chapter(&mut author, &chapter, version, "The road was open and the sky was wide.").await;
    ...
    assert_eq!(body["sanitized_html"], "<p>The road was open and the sky was wide.</p>");
...
async fn progress_saved_by_one_device_does_not_overwrite_another() {
    ...
    save_chapter(&mut reader, &chapter, version, "A chapter with enough words to have a middle.").await;
    ...
    let account_id: lorehaven_domain::AccountId = reader
        .get("/api/v1/auth/me")
        .await
        .1["account"]["id"]
        .as_str()
...
async fn a_private_rating_changes_no_public_number() {
    ...
    let (status, _) = reader
        .put(&format!("/api/v1/works/{work_id}/rating"), json!({ "stars": 5, "is_public": false }))
        .await;
    assert_eq!(status, StatusCode::OK);

    // The public aggregate is still absent.
    let summary = reading::public_rating_summary(
        &harness.db,
        work_id.parse().expect("work id"),
    )
    .await
    .expect("summary");
    assert!(summary.is_none(), "a private rating must not move the aggregate");
...
async fn the_public_aggregate_is_absent_below_the_minimum_count() {
    ...
    for i in 0..4 {
        ...
        register(&mut rater, &format!("rater{i}@example.com"), &format!("Rater{i}")).await;
        let pseud = active_pseud(&mut rater).await;
        reading::upsert_rating(
            &harness.db,
            rater.get("/api/v1/auth/me").await.1["account"]["id"].as_str().unwrap().parse().unwrap(),
    ...
    let summary = reading::public_rating_summary(&harness.db, work_uuid).await.expect("summary");
    assert!(summary.is_none(), "below the minimum count the aggregate is hidden");
...
async fn switching_pseud_shows_a_different_history() {
    ...
    let (status, body) = reader.post("/api/v1/pseuds", json!({ "handle": "Second" })).await;
    ...
    let (status, _) = reader.post(&format!("/api/v1/pseuds/{second}/activate"), json!({})).await;
    assert_eq!(status, StatusCode::NO_CONTENT);
    ...
    reading::touch_history(
        &harness.db,
        reader.get("/api/v1/auth/me").await.1["account"]["id"].as_str().unwrap().parse().unwrap(),
        pseud_id,
        "work",
        &work_id,
    ...
    let (status, _) = reader.post(&format!("/api/v1/pseuds/{first}/activate"), json!({})).await;
    assert_eq!(status, StatusCode::NO_CONTENT);
    let (status, body) = reader.get("/api/v1/library/history").await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["items"].as_array().expect("items").len(), 0, "the first pseud has not read it");
...
async fn clearing_history_removes_only_the_callers_rows() {
    ...
    let account_a: lorehaven_domain::AccountId = reader_a.get("/api/v1/auth/me").await.1["account"]["id"].as_str().unwrap().parse().unwrap();

    let mut reader_b = harness.client();
    register(&mut reader_b, "b@example.com", "BBB").await;
    let account_b: lorehaven_domain::AccountId = reader_b.get("/api/v1/auth/me").await.1["account"]["id"].as_str().unwrap().parse().unwrap();

    // Both touch history for the same subject.
    reading::touch_history(&harness.db, account_a, active_pseud(&mut reader_a).await.parse().expect("pseud"), "work", "work-1", None).await.expect("touch a");
    reading::touch_history(&harness.db, account_b, active_pseud(&mut reader_b).await.parse().expect("pseud"), "work", "work-1", None).await.expect("touch b");

    // A clears their history.
    let (status, _) = reader_a.post("/api/v1/library/history/clear", json!({})).await;
    assert_eq!(status, StatusCode::NO_CONTENT);

    // A's history is empty; B's still has its row.
    let a_history = reading::history_for(&harness.db, account_a, active_pseud(&mut reader_a).await.parse().expect("pseud"), 50).await.expect("history a");
    let b_history = reading::history_for(&harness.db, account_b, active_pseud(&mut reader_b).await.parse().expect("pseud"), 50).await.expect("history b");
