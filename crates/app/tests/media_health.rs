...
async fn admin_overview_mixed_health() {
    ...
    media_resilience::insert_media_reference(db, "ref-001", "hash-1", lorehaven_domain::media_resilience::MediaKind::Image)
        .await.expect("insert ref-001");
    for i in 0..3 {
        media_resilience::insert_availability_link(
            db, &format!("link-001-{i}"), "ref-001",
            &format!("https://example.com/img-001-{i}.png"),
            lorehaven_domain::media_resilience::LinkProvider::Imgur, None, 100,
        ).await.expect("insert link");
        media_resilience::update_link_status(
            db, &format!("link-001-{i}"),
            lorehaven_domain::media_resilience::LinkStatus::Healthy, 0,
        ).await.expect("update link");
    }

    // ref-002: 1 healthy link (below threshold)
    media_resilience::insert_media_reference(db, "ref-002", "hash-2", lorehaven_domain::media_resilience::MediaKind::Image)
        .await.expect("insert ref-002");
    media_resilience::insert_availability_link(
        db, "link-002", "ref-002",
        "https://example.com/img-002.png",
        lorehaven_domain::media_resilience::LinkProvider::Pinterest, None, 50,
    ).await.expect("insert link");
    media_resilience::update_link_status(
        db, "link-002",
        lorehaven_domain::media_resilience::LinkStatus::Healthy, 0,
    ...
    media_resilience::insert_media_reference(db, "ref-003", "hash-3", lorehaven_domain::media_resilience::MediaKind::Image)
        .await.expect("insert ref-003");
    for i in 0..2 {
        media_resilience::insert_availability_link(
            db, &format!("link-003-{i}"), "ref-003",
            &format!("https://example.com/img-003-{i}.png"),
            lorehaven_domain::media_resilience::LinkProvider::Tumblr, None, 50,
        ).await.expect("insert link");
        media_resilience::update_link_status(
            db, &format!("link-003-{i}"),
            lorehaven_domain::media_resilience::LinkStatus::Healthy, 0,
        ).await.expect("update link");
    }

    let total = media_resilience::count_total_references(db).await.expect("count");
    assert_eq!(total, 3);

    let well_mirrored = media_resilience::count_well_mirrored(db, 3).await.expect("count");
    assert_eq!(well_mirrored, 1);

    let below = media_resilience::count_references_below_threshold(db, 3).await.expect("count");
    assert_eq!(below, 2);
}
...
async fn link_rot_by_provider() {
    ...
    media_resilience::insert_media_reference(db, "ref-rot-1", "hash-r1", lorehaven_domain::media_resilience::MediaKind::Image)
        .await.expect("insert");

    media_resilience::insert_availability_link(
        db, "link-r1", "ref-rot-1", "https://imgur.com/dead1",
        lorehaven_domain::media_resilience::LinkProvider::Imgur, None, 50,
    ).await.expect("insert link");

    // Simulate rot: update status to dead (last_healthy_at remains NULL since never healthy)
    media_resilience::update_link_status(
        db, "link-r1",
        lorehaven_domain::media_resilience::LinkStatus::Dead, 5,
    ).await.expect("update");
...
async fn curator_leaderboard_with_entries() {
    ...
    media_resilience::insert_media_reference(db, "ref-lb", "hash-lb", lorehaven_domain::media_resilience::MediaKind::Image)
        .await.expect("insert");
    media_resilience::insert_availability_link(
        db, "link-lb", "ref-lb", "https://example.com/lb.png",
        lorehaven_domain::media_resilience::LinkProvider::Other, None, 50,
    ).await.expect("insert link");

    media_resilience::insert_curator_reward(
        db, "alice", lorehaven_domain::media_resilience::CuratorAction::MirrorAdd,
        Some("ref-lb"), Some("link-lb"), 15,
    ).await.expect("reward alice");

    media_resilience::insert_curator_reward(
        db, "bob", lorehaven_domain::media_resilience::CuratorAction::Verify,
        Some("ref-lb"), Some("link-lb"), 5,
    ).await.expect("reward bob");
...
async fn provider_reliability_empty() {
    ...
    let providers = media_resilience::provider_reliability(db)
        .await.expect("provider reliability");
...
async fn provider_reliability_with_data() {
    ...
    media_resilience::insert_media_reference(db, "ref-pr-1", "hash-pr1", lorehaven_domain::media_resilience::MediaKind::Image)
        .await.expect("insert");

    // 2 healthy links on provider A
    for i in 0..2 {
        media_resilience::insert_availability_link(
            db, &format!("link-pa-{i}"), "ref-pr-1",
    ...
    media_resilience::insert_availability_link(
        db, "link-pb", "ref-pr-1", "https://provider-b.com/img.png",
        lorehaven_domain::media_resilience::LinkProvider::Pinterest, None, 50,
    ).await.expect("insert link");
    media_resilience::update_link_status(
        db, "link-pb",
        lorehaven_domain::media_resilience::LinkStatus::Dead, 3,
    ).await.expect("update");
...
async fn admin_overview_route_operator_ok() {
    ...
    let _account_id = create_account(db, "op@test", AgeState::DeclaredAdult, AccountStatus::Active)
        .await
