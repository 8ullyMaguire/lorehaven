...
async fn fetch_balances_sqlite(pool: &sqlx::SqlitePool, account: &str) -> Result<Vec<(String, i64)>, sqlx::Error> {
    let rows = sqlx::query("SELECT bucket, SUM(amount_bp) AS total FROM credit_entries WHERE account = ? GROUP BY bucket")
        .bind(account)
        .fetch_all(pool)
        .await?;
    Ok(rows.iter().map(|r| (r.get::<String, _>("bucket"), r.get::<i64, _>("total"))).collect())
}

async fn fetch_balances_postgres(pool: &sqlx::postgres::PgPool, account: &str) -> Result<Vec<(String, i64)>, sqlx::Error> {
...
async fn fetch_queue_sqlite(pool: &sqlx::SqlitePool, job_id: &str) -> Result<Option<(String, i64)>, sqlx::Error> {
    let row = sqlx::query("SELECT priority_class, position FROM queue_slots WHERE job_id = ?")
        .bind(job_id)
        .fetch_optional(pool)
        .await?;
    Ok(row.map(|r| (r.get::<String, _>("priority_class"), r.get::<i64, _>("position"))))
}

async fn fetch_queue_postgres(pool: &sqlx::postgres::PgPool, job_id: &str) -> Result<Option<(String, i64)>, sqlx::Error> {
...
async fn fetch_usage_sqlite(pool: &sqlx::SqlitePool, account: &str, day: &str) -> Result<Vec<(String, i64)>, sqlx::Error> {
    let rows = sqlx::query("SELECT action, count FROM usage_counters WHERE account = ? AND day = ?")
        .bind(account).bind(day)
        .fetch_all(pool)
        .await?;
    Ok(rows.iter().map(|r| (r.get::<String, _>("action"), r.get::<i64, _>("count"))).collect())
}

async fn fetch_usage_postgres(pool: &sqlx::postgres::PgPool, account: &str, day: &str) -> Result<Vec<(String, i64)>, sqlx::Error> {
    let rows = sqlx::query("SELECT action, count FROM usage_counters WHERE account = $1 AND day = $2")
        .bind(account).bind(day)
        .fetch_all(pool)
        .await?;
    Ok(rows.iter().map(|r| (r.get::<String, _>("action"), r.get::<i64, _>("count"))).collect())
...
pub async fn post_transaction(
    ...
    match db.backend() {
        Backend::Sqlite => {
            ...
            .bind(&txn_id).bind(txn_type.as_str()).bind(idempotency_key).bind(reference).bind(&now)
...
pub async fn balances(db: &Database, account: &str) -> Result<Vec<(String, i64)>, sqlx::Error> {
    match db.backend() {
        ...
        Backend::Postgres => fetch_balances_postgres(db.postgres_pool().expect("postgres"), account).await,
    }
}
    ...
    match db.backend() {
        Backend::Sqlite => {
            sqlx::query("UPDATE credit_holds SET captured_at = ?, amount = ? WHERE id = ?")
                .bind(&now).bind(actual).bind(hold_id)
                .execute(db.sqlite_pool().expect("sqlite")).await?;
        }
        Backend::Postgres => {
...
pub async fn enqueue_job(
    ...
    match db.backend() {
        Backend::Sqlite => {
            ...
            .bind(job_id).bind(priority_class).bind(position).bind(&now)
            .execute(db.sqlite_pool().expect("sqlite")).await?;
        }
        Backend::Postgres => {
            sqlx::query(
...
pub async fn queue_position(db: &Database, job_id: &str) -> Result<Option<(String, i64)>, sqlx::Error> {
    match db.backend() {
        Backend::Sqlite => fetch_queue_sqlite(db.sqlite_pool().expect("sqlite"), job_id).await,
        Backend::Postgres => fetch_queue_postgres(db.postgres_pool().expect("postgres"), job_id).await,
    }
}
...
pub async fn bump_counter(
    ...
    let count: i64 = match db.backend() {
        Backend::Sqlite => {
            ...
            .bind(account).bind(action).bind(day)
...
pub async fn usage_for(db: &Database, account: &str, day: &str) -> Result<Vec<(String, i64)>, sqlx::Error> {
    match db.backend() {
        Backend::Sqlite => fetch_usage_sqlite(db.sqlite_pool().expect("sqlite"), account, day).await,
        Backend::Postgres => fetch_usage_postgres(db.postgres_pool().expect("postgres"), account, day).await,
    }
}
